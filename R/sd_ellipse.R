...
sd_ellipse <- function (points = NULL, x = NULL, y = NULL,
  ...
  if (weighted) {
    sigmax <-
      sqrt(2) * sqrt((sum(
        colSums(weights * tmpAc2) * c(cos2theta, sin2theta)
      ) -
        2 * sum(weights * tmpAcp) * sinthetacostheta) / sum(weights))

    sigmay <-
      sqrt(2) * sqrt((sum(
        colSums(weights * tmpAc2) * c(sin2theta, cos2theta)
      ) +
        2 * (sum(weights * tmpAcp)) *  sinthetacostheta) / sum(weights))
  } else {
    sigmax <-
  ...
  ca <- cos(al)
  sa <- sin(al)
  coordsSDE <- t(t(xy %*% rbind(c(ca, sa), c(-sa, ca))) + centre)
  ...
  result.sde <- as.data.frame(rbind(
    c(weighted = weighted, centre = centre, Sigma.x = sigmax,
